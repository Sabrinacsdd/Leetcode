...
class TimeMap {
    ...
    public String get(String key, int timestamp) {
        ...
        Integer r = map.get(key).floorKey(timestamp);
        return r != null ? map.get(key).get(r) : "";
    }
}
 ...
 */
 ```
 ## Complexity Analyze
Time:  O(1) for each set operation, and O(logN) for each get operation, where NN is the number of entries in the TimeMap.
Space: O(n)