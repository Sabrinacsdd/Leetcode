    ...
    public int maxProfit(int[] inventory, int orders) {
        ...
        while(orders > 0){
            if(n > 0 && inventory[n] - inventory[n-1] > 0 && orders >= count * (inventory[n] - inventory[n-1])){
                ans += count * ((inventory[n] * (long)(inventory[n]+1))/2 - (inventory[n-1] * (long)(inventory[n-1]+1)) / 2);
                orders -= count * (inventory[n] - inventory[n-1]);
            }else if(n == 0 || inventory[n] - inventory[n-1] > 0){
                long a = orders / count;
                ans += count * ((inventory[n] * (long)(inventory[n]+1))/2 - ((inventory[n]-a) * (long)(inventory[n]-a+1)) / 2);
                long b = orders % count;
                ans += b * (inventory[n]-a);
                orders = 0;
            }
